...
DEFAULT_PARAMS = {
    ...
    'max_u': [0.15, 0.05, np.radians(15), np.radians(15)],
    'min_u': [-0.15, -0.05, np.radians(-15), np.radians(-15)],
    'bias_u': [0.2, 0.1, 0., 0.],
    # ddpg
    ...
    'batch_size': 256*3,  # per mpi thread, measured in transitions and reduced to even multiple of chunk_length.
    'n_test_rollouts': 10,  # number of test rollouts per epoch, each consists of rollout_batch_size rollouts
    'test_with_polyak': False,  # run test episodes with the target network
    # exploration
    'random_eps': 0.3,  # percentage of time a random action is taken
    'noise_eps': 0.02,  # std of gaussian noise added to not-completely-random actions as a percentage of max_u
    # HER
...
def prepare_params(kwargs):
    ...
    kwargs['min_u'] = np.array(kwargs['min_u']) if isinstance(kwargs['min_u'], list) else kwargs['min_u']
    kwargs['bias_u'] = np.array(kwargs['bias_u']) if isinstance(kwargs['bias_u'], list) else kwargs['bias_u']
    ...
    for name in ['buffer_size', 'hidden', 'layers',
                 ...
                 'norm_eps', 'norm_clip', 'max_u', 'min_u', 'bias_u',
